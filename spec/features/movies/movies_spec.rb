...
RSpec.describe 'Movies Show Page', type: :feature do
  ...
  it "discover top rated movies button leads to the movies page" do
    ...
    within ".movie_reviews" do
      ...
      expect(page).to have_content ("Dune: Part Two surpasses even the highest expectations, establishing itself as an unquestionable technical masterpiece of blockbuster filmmaking.")
      expect(page).to have_content("Review by: r96sk")
      expect(page).to have_content ("My interest did wane slightly at roughly the middle part, as was similarly the case with the first film in truth, but that was a feeling that only lasted for a relativiely short time.")
    end
  end
